...
namespace WxBot
{
    internal class WechatBot
    {
        ...
        private async Task GetPublicContact()
        {
            var url =
                $"{_baseUri}/webwxbatchgetcontact?type=ex&r={Common.ConvertDateTimeToInt(DateTime.Now)}&lang=zh_CN&pass_ticket={_loginXml.PassTicket}";
        }
        ...
        /// </summary>
        /// <returns></returns>
        protected virtual async Task Schedule()
        {
            using (var client = new HttpClient())
            {
                var response = await client.GetAsync("http://syzb.qianjifang.com.cn/api/Monitor");
                var result = await response.Content.ReadAsAsync<MonitorResponseModel>();
                if (result != null)
                {
                    var users = _contactList.Where(p => result.UserName.Contains(p.RemarkName));
                    foreach (var user in users)
                        await SendMessage(result.Content, user.UserName);
                }
            }
